...
# relabel the Spanish gate stats on the Live-cells plot -> English (Vivas/Muertas -> Live/Dead)
_v = plots["VIVAS"]; _dd = ImageDraw.Draw(_v); _lf = font(30, bold=False)
_dd.rectangle([26, 108, 143, 182], fill="white")     # cover "Vivas 91,4" (left margin, outside gate)
_dd.rectangle([20, 356, 164, 432], fill="white")     # cover "Muertas 7,82" (incl. trailing s)
_dd.text((36, 116), "Live", fill="black", font=_lf, anchor="la")
_dd.text((46, 150), "91.4", fill="black", font=_lf, anchor="la")
_dd.text((32, 364), "Dead", fill="black", font=_lf, anchor="la")
_dd.text((46, 398), "7.82", fill="black", font=_lf, anchor="la")

# CD45 split: relabel quadrant stats in the same font (right-aligned, clear of the gate)
_c = plots["CD45"]; _dc = ImageDraw.Draw(_c); _cf = font(28, bold=False)   # gate line at x=203
_dc.rectangle([60, 82, 200, 158], fill="white"); _dc.rectangle([60, 352, 200, 428], fill="white")
_dc.text((196, 92), "CD45+", fill="black", font=_cf, anchor="ra"); _dc.text((196, 124), "8.01", fill="black", font=_cf, anchor="ra")
_dc.text((196, 362), "CD45-", fill="black", font=_cf, anchor="ra"); _dc.text((196, 394), "87.6", fill="black", font=_cf, anchor="ra")

# EpCAM: original label overlapped the gate line (x=193) -> clear the strip and redraw the line
_e = plots["EPCAM"]; _de = ImageDraw.Draw(_e); _ef = font(28, bold=False)
_de.rectangle([56, 158, 202, 236], fill="white")
_de.rectangle([193, 158, 195, 236], fill="black")           # restore gate left-edge segment
_de.text((188, 168), "EpCAM+", fill="black", font=_ef, anchor="ra"); _de.text((188, 200), "15.6", fill="black", font=_ef, anchor="ra")
...
gutter, colgap, rowgap, margin, cgap = 60, 60, 0, 40, 22
GATE_H = 520                                             # every gating layout: same height
# both graphs share one box, sized to the two-gate stack height (VIVAS+CD45) so the CD45 graph
# aligns top with the Live/Dead layout and both graphs are identical in size
_gh = 2 * GATE_H + cgap
GBOX = (round(_gh * 2400 / 1560), _gh)                   # graph PNGs are 2400x1560 (8x5.2 in)
fl = font(60)
...
for letter, gb, gr in built:
    ...
    gy = y                                               # gate block top-aligned
    canvas.paste(gb, (gx, gy))
    canvas.paste(gr, (xr, y + rh - gr.height))           # graph bottom-aligned -> rows read as adjacent
    d.text((margin + 2, gy - 4), letter, fill="black", font=fl, anchor="la")  # top-left corner
    y += rh + rowgap

out = f"{FIG}/Fig_viab_epcam.png"
